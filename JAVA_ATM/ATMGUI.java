...
public class ATMGUI extends JFrame {
    ...
    private JPanel buildAmountDisplayGrid() {
        ...
        for (String lbl : labels) {
            grid.add(makeAmountDisplayLabel(lbl));
        }
    ...
    private JPanel buildKeypadPanel() {
        ...
        // Row 1
        grid.add(digitBtns[7]);
        grid.add(digitBtns[8]);
        grid.add(digitBtns[9]);
        grid.add(cancelBtn);

        // Row 2
        grid.add(digitBtns[4]);
        grid.add(digitBtns[5]);
        grid.add(digitBtns[6]);
        grid.add(clearBtn);

        // Row 3
        grid.add(digitBtns[1]);
        grid.add(digitBtns[2]);
        grid.add(digitBtns[3]);
        grid.add(enterBtn);

        // Row 4
        grid.add(digitBtns[0]);
        grid.add(dotBtn);
        grid.add(doubleZeroBtn);
        grid.add(new JLabel());
    ...
    private void setAtmState(int state) {
        ...
        /*
         * The keypad is enabled in STATE_POST_WITHDRAW so the user can type:
         * 1 + Enter, 2 + Enter, or 3 + Enter.
         */
        ...
        switch (state) {
            ...
            case STATE_POST_WITHDRAW:
                ...
                appendDisplay("");
                appendDisplay(" Type 1, 2, or 3 and press Enter.");
                setStatus("Use numpad: 1, 2, or 3, then Enter");
                break;
    ...
    private void runATMSession() {
        ...
        for (pinAttempts = 0; pinAttempts < 3; pinAttempts++) {
            ...
            if (att > 0) {
                SwingUtilities.invokeLater(() -> {
                    ...
                    appendDisplay("-------- PIN ENTRY --------");
                    appendDisplay("");
                    appendDisplay(" Wrong PIN - attempt " + (att + 1) + " of 3");
                    appendDisplay("");
                    appendDisplay(" Re-enter PIN and press Enter.");
                });
            }
        ...
        if (!authenticated) {
            SwingUtilities.invokeLater(() -> {
                ...
                appendDisplay("");
                appendDisplay(" Too many wrong PIN attempts.");
    ...
    private boolean doWithdraw() {
        ...
        try {
            currencyChoice = Integer.parseInt(currencyInput.trim());
        } catch (NumberFormatException e) {
            currencyChoice = 0;
        }
        ...
        // -------------------------------------------------
        // Step 6: Debit account only.
        // Cash is NOT dispensed yet.
        // It will be dispensed after card ejection if user logs out.
        // -------------------------------------------------
        bankDatabase.debit(currentAccountNumber, withdrawalInHKD);

        clearDisplay();
        appendDisplay("-------- WITHDRAWAL APPROVED --------");
        guiScreen.displayMessageLine("\n Withdrawal approved.");
        ...
        switch (postChoice) {
            ...
            case 2:
                clearDisplay();
                appendDisplay("-------- CONTINUE SESSION --------");
                appendDisplay("");
                appendDisplay(" Please remember to take your cash later");
                appendDisplay(" when you finish your session.");
                appendDisplay("");
    ...
    // ----------------------------------------------------------
    // Complete withdrawal logout sequence
    // ----------------------------------------------------------
    private void completeWithdrawalLogoutSequence(double amount, String currency) {
    ...
    // ----------------------------------------------------------
    private void printWithdrawalReceipt(
            String currency,
            double amount,
            double amountInHKD,
            double balance) {
        ...
        guiScreen.displayMessageLine("\n ====== Withdrawal Receipt ======");
        guiScreen.displayMessageLine(" Account : " + currentAccountNumber);
    ...
    private void setKeypadEnabled(boolean on) {
        SwingUtilities.invokeLater(() -> {
            ...
            if (dotBtn != null) {
                dotBtn.setEnabled(on);
            }

            if (doubleZeroBtn != null) {
                doubleZeroBtn.setEnabled(on);
            }

            if (enterBtn != null) {
                enterBtn.setEnabled(on);
            }
